...
class LowBaseStrategy(BaseStrategy):
    ...
    def generate_signals(self, df: pd.DataFrame) -> List[Tuple[str, str]]:
        ...
        price_to_atr_ratio = (
            latest["close"] / latest["ATR14"] if latest["ATR14"] > 0 else float("inf")
        )
        ...
        below_sma50 = (
            latest["close"] < latest["SMA50"] if "SMA50" in df.columns else True
        )
        below_sma200 = (
            latest["close"] < latest["SMA200"] if "SMA200" in df.columns else True
        )

        # Generate signal if conditions are met
        if (
            price_to_atr_ratio < self.min_atr_ratio
            and rsi_below_threshold
            and below_sma50
            and below_sma200
        ):
            log_info(
                f"Low Base signal generated for {symbol}. "
