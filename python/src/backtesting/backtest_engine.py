...
class BacktestEngine:
    ...
    def run_backtest(
        ...
        for symbol in symbols:
            ...
            all_data[symbol] = self._get_mock_data(
                symbol, start_date, end_date, data_frequency
            )
        ...
        all_dates = sorted(
            set(date for symbol_data in all_data.values() for date in symbol_data.index)
        )
            ...
            for symbol in list(positions.keys()):
                ...
                if exit_reason:
                    ...
                    exit_price = self._simulate_exit_price(
                        trade, current_price, exit_reason
                    )
        ...
        logger.info(
            f"Completed backtest in {time.time() - start_time:.2f}s with {len(trades)} trades"
        )
        return result
    ...
    def run_optimization(
        ...
        **kwargs,
        ...
        if method == "grid_search":
            ...
            param_sets = self._generate_random_parameters(
                parameter_ranges, n_iterations
            )
            ...
            bounds = [
                (min_val, max_val) for _, (min_val, max_val) in parameter_ranges.items()
            ]
            ...
            if method == "differential_evolution":
                ...
                try:
                    ...
                    logger.warning(
                        "scikit-optimize not installed, falling back to random search"
                    )
        ...
        logger.info(
            f"Completed optimization in {optimization_result.time_taken:.2f}s with {len(all_results)} parameter combinations"
        )
    ...
    def save_backtest_result(
        self, result: BacktestResult, output_dir: str = None
    ) -> str:
        ...
        filename = (
            f"{result.strategy_name}_{','.join(result.symbols)}_{result.run_id}.pkl"
        )
    ...
    def save_optimization_result(
        self, result: OptimizationResult, output_dir: str = None
    ) -> str:
        ...
        # Generate unique filename
        filename = (
            f"opt_{result.strategy_name}_{','.join(result.symbols)}_{result.run_id}.pkl"
        )
        filepath = os.path.join(output_dir, filename)
    ...
    def _get_mock_data(
        self, symbol: str, start_date: str, end_date: str, data_frequency: str = "daily"
    ) -> pd.DataFrame:
    ...
    def _calculate_stop_price(
        self, data: pd.DataFrame, direction: str, atr_mult: float
    ) -> float:
    ...
    def _check_exit_conditions(
        ...
        if self.config.USE_TRAILING_STOP and hasattr(trade, "highest_price"):
            ...
            if trade.direction == "LONG":
                if (
                    not hasattr(trade, "highest_price")
                ...
                if (
                    not hasattr(trade, "lowest_price")
    ...
    def _simulate_exit_price(
        self, trade: TradeResult, current_price: float, exit_reason: str
    ) -> float:
    ...
    def _calculate_performance_metrics(
        self, trades: List[TradeResult], equity_curve: pd.Series, drawdowns: pd.Series
        ...
        profit_factor = (
            abs(gross_profits / gross_losses) if gross_losses != 0 else float("inf")
        )
        ...
        sortino_ratio = (
            (excess_returns.mean() / negative_returns.std()) * np.sqrt(252)
        ...
        return {
            ...
            "avg_loss": avg_loss,
            "avg_profit": (
                (gross_profits + gross_losses) / total_trades if total_trades > 0 else 0
            ),
    ...
    def _generate_parameter_grid(
        self, parameter_ranges: Dict[str, Tuple[float, float]], n_points: int
    ) -> List[Dict[str, float]]:
    ...
    def _generate_random_parameters(
        self, parameter_ranges: Dict[str, Tuple[float, float]], n_samples: int
    ) -> List[Dict[str, float]]:
