...
def parse_arguments():
    ...
    parser.add_argument(
        "--symbols", type=str, nargs="+", help="List of symbols to backtest"
    )
    parser.add_argument(
        "--strategies", type=str, nargs="+", help="List of strategies to backtest"
    )
    ...
    parser.add_argument("--output", type=str, help="Output file for backtest report")
...
def main():
    ...
    if not args.symbols:
        args.symbols = config.get(
            "BACKTEST_SYMBOLS", ["AAPL", "MSFT", "GOOGL", "AMZN", "META"]
        )
    ...
    log_info(
        f"Starting backtest for {len(args.symbols)} symbols "
    ...
    if results and "overall" in results:
        ...
        print(
            f"Total profit: ${overall['total_profit']:.2f} ({overall['total_profit']/overall['initial_equity']*100:.2f}%)"
        )
