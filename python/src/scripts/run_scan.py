...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run a market scan using trading strategies"
    )
    ...
    parser.add_argument(
        "--symbols", type=str, nargs="+", help="List of symbols to scan"
    )
    parser.add_argument(
        "--strategies", type=str, nargs="+", help="List of strategies to apply"
    )
    parser.add_argument(
        "--use-go-scanner", action="store_true", help="Use the Go scanner service"
    )
...
def save_results(results: Dict[str, List[str]], output_file: str) -> None:
...
def main() -> None:
    ...
    if not args.symbols:
        args.symbols = config.get(
            "SCAN_SYMBOLS", ["AAPL", "MSFT", "GOOGL", "AMZN", "META"]
        )
