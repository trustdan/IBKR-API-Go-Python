...
from ..trading.option_selector import OptionSelector
from ..trading.trade_executor import TradeExecutor
from ..trading.risk_manager import RiskManager
from ..brokers.ibkr_api import IBKRApi
from ..app.scanner_client import ScannerClient
...
class Trader:
    ...
    def scan_for_signals(self, symbols: List[str], strategies: List[str]) -> Dict[str, List[str]]:
        ...
        """
        try:
            log_info(f"Scanning {len(symbols)} symbols with {len(strategies)} strategies")
    ...
    def process_signal(self, signal: Dict[str, Any]) -> Tuple[str, Any]:
        ...
        try:
            ...
            if position_size <= 0:
                return "REJECTED", "Position size calculation resulted in zero contracts"
    ...
    def run_scan_cycle(self, symbols: List[str], strategies: List[str]) -> Dict[str, Any]:
        ...
        while not self.signal_queue.empty() and (time.time() - start_time) < 300:  # 5-minute timeout
            time.sleep(1.0)
        ...
        return {
            ...
            "signals_found": sum(len(signal_types) for signal_types in signals.values()),
            "status": status
        } 