...
class ErrorHandler:
    ...
    def __init__(self, config, alert_system=None):
        ...
        critical_components = [
            "IBKR_API", "DATA_PROVIDER", "OPTION_SELECTOR", 
    ...
    def handle_error(self, component: str, error_type: str, 
                   error_msg: str, context: Optional[Dict] = None) -> Tuple[bool, str]:
        """
        ...
        if component in self.circuit_breakers and self.circuit_breakers[component]["tripped"]:
        ...
        if self.is_critical_error(error_type) or self.error_counts[error_key] >= self.config.ERROR_THRESHOLD:
            if self.alert_system:
                self.alert_system.send_system_alert(
                    component,
                    "ERROR",
                    f"{error_type}: {error_msg}\n{stack_trace if self.config.INCLUDE_STACK_TRACE_IN_ALERTS else ''}"
                )
            
            # Trip circuit breaker if critical and repeated errors
            if self.is_critical_error(error_type) and self.error_counts[error_key] >= self.config.CIRCUIT_BREAKER_THRESHOLD:
        ...
        if self.recovery_attempts[error_key] < self.config.MAX_RECOVERY_ATTEMPTS:
            ...
            if self.alert_system:
                self.alert_system.send_system_alert(
                    component,
                    "RECOVERY_FAILED",
                    f"Max recovery attempts ({self.config.MAX_RECOVERY_ATTEMPTS}) reached for {error_type}"
                )
            return False, f"Max recovery attempts reached ({self.config.MAX_RECOVERY_ATTEMPTS})"
    ...
    def attempt_recovery(self, component: str, error_type: str, 
        ...
        log_info(f"Attempting recovery for {component}:{error_type} (attempt {self.recovery_attempts[error_key]})")
    ...
    def _recover_ibkr_api(self, error_type: str, context: Optional[Dict] = None) -> Tuple[bool, str]:
        """
        ...
        if error_type == "CONNECTION_ERROR":
            ...
            try:
                ...
                backoff_seconds = min(2 ** retry_count, 60)  # Exponential backoff, max 60 seconds
    ...
    def _recover_data_provider(self, error_type: str, context: Optional[Dict] = None) -> Tuple[bool, str]:
        """
    ...
    def _recover_option_selector(self, error_type: str, context: Optional[Dict] = None) -> Tuple[bool, str]:
        """
    ...
    def _recover_trade_executor(self, error_type: str, context: Optional[Dict] = None) -> Tuple[bool, str]:
        """
    ...
    def _recover_risk_manager(self, error_type: str, context: Optional[Dict] = None) -> Tuple[bool, str]:
        """
    ...
    def _generic_recovery(self, component: str, error_type: str, 
                        context: Optional[Dict] = None) -> Tuple[bool, str]:
        """
    ...
    def _trip_circuit_breaker(self, component: str) -> None:
        ...
        now = datetime.now()
        reset_time = now + timedelta(minutes=self.config.CIRCUIT_BREAKER_MINUTES)
    ...
    def clear_error_counts(self, older_than_minutes: int = 60) -> int:
        ...
        for error_key in list(self.error_counts.keys()):
            if (error_key in self.last_errors and 
                self.last_errors[error_key] < threshold_time):
