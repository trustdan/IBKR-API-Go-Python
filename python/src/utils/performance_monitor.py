...
class PerformanceMonitor:
    ...
    resource_monitor_thread: Optional[threading.Thread]
    ...
    def __init__(
        ...
        self.resource_monitoring = getattr(
    ...
    def _get_thresholds(self) -> Dict[str, Dict[str, float]]:
        ...
        thresholds = {
            ...
            "option_selection": {
                "warning": getattr(
                    self.config, "OPTION_SELECTION_WARNING_THRESHOLD", 2.0
                ),
                "critical": getattr(
                    self.config, "OPTION_SELECTION_CRITICAL_THRESHOLD", 10.0
                ),
            },
            "order_execution": {
                "warning": getattr(
                    self.config, "ORDER_EXECUTION_WARNING_THRESHOLD", 2.0
                ),
                "critical": getattr(
                    self.config, "ORDER_EXECUTION_CRITICAL_THRESHOLD", 5.0
                ),
            },
            "data_processing": {
                "warning": getattr(
                    self.config, "DATA_PROCESSING_WARNING_THRESHOLD", 1.0
                ),
                "critical": getattr(
                    self.config, "DATA_PROCESSING_CRITICAL_THRESHOLD", 5.0
                ),
            },
            "scanning": {
            ...
            "system": {
                ...
                "memory_warning": getattr(
                    self.config, "MEMORY_WARNING_THRESHOLD", 80.0
                ),
                "memory_critical": getattr(
                    self.config, "MEMORY_CRITICAL_THRESHOLD", 95.0
                ),
            },
        }
    ...
    def _check_resource_thresholds(
        ...
        if cpu_percent >= self.thresholds["system"]["cpu_critical"]:
            log_error(
                f"CRITICAL: CPU usage at {cpu_percent}% (threshold: {self.thresholds['system']['cpu_critical']}%)"
            )
            ...
            log_warning(
                f"WARNING: CPU usage at {cpu_percent}% (threshold: {self.thresholds['system']['cpu_warning']}%)"
            )
            if self.alert_system:
                self.alert_system.send_performance_alert(
                    "CPU Usage",
        ...
        if memory_percent >= self.thresholds["system"]["memory_critical"]:
            log_error(
                f"CRITICAL: Memory usage at {memory_percent}% (threshold: {self.thresholds['system']['memory_critical']}%)"
            )
            if self.alert_system:
                self.alert_system.send_performance_alert(
                    "Memory Usage",
            ...
            log_warning(
                f"WARNING: Memory usage at {memory_percent}% (threshold: {self.thresholds['system']['memory_warning']}%)"
            )
            if self.alert_system:
                self.alert_system.send_performance_alert(
                    "Memory Usage",
    ...
    def start_timer(self, component: str, operation: str) -> str:
        ...
        """
        timer_id = (
            f"{component}:{operation}:{datetime.now().strftime('%Y%m%d%H%M%S%f')}"
        )
    ...
    def record_operation_time(
        self, component: str, operation: str, execution_time: float
    ) -> None:
        """
        ...
        if execution_time > 1.0:  # Only log operations taking more than 1 second
            log_debug(
                f"{component.capitalize()} {operation} completed in {execution_time:.2f}s"
            )

    def _check_execution_threshold(
        self, component: str, operation: str, execution_time: float
    ) -> None:
        """
        ...
        elif component == "trade_executor" and operation in [
            "execute_trade",
            "place_order",
        ]:
        ...
        if (
            "critical" in self.thresholds[threshold_category]
            and execution_time >= self.thresholds[threshold_category]["critical"]
        ):
            log_error(
        ...
        # Check against warning threshold
            ...
            "warning" in self.thresholds[threshold_category]
            and execution_time >= self.thresholds[threshold_category]["warning"]
        ):
            log_warning(
    ...
    def record_metric(self, category: str, name: str, value: float) -> None:
        ...
        """
        key = f"{category}:{name}"
        self.metrics[key].append({"timestamp": datetime.now(), "value": value})
            ...
            if ":" in key:
                ...
                if category in [
                    ...
                    summary["operations"][category][name] = {
                        ...
                        "p95": (
                            self._percentile(times, 95) if len(times) >= 20 else None
                        ),
                    }
    ...
    def run_health_check(self) -> Dict[str, str]:
        ...
        if (
            system_status["cpu_percent"] > self.thresholds["system"]["cpu_critical"]
            or system_status["memory_percent"]
            > self.thresholds["system"]["memory_critical"]
        ):
            health["system"] = "CRITICAL"
        elif (
            system_status["cpu_percent"] > self.thresholds["system"]["cpu_warning"]
            or system_status["memory_percent"]
            > self.thresholds["system"]["memory_warning"]
        ):
        ...
        if not api_operations:
            ...
            recent_errors = sum(
                1 for k in api_operations if k.endswith("error") and self.metrics[k]
            )
            if recent_errors > 3:
                health["api"] = "CRITICAL"
        ...
        if scanner_key in self.metrics and self.metrics[scanner_key]:
            recent_scans = [
                v
                for v in self.metrics[scanner_key]
                if v["timestamp"] >= datetime.now() - timedelta(hours=1)
            ]
            if not recent_scans:
                health["scanner"] = "WARNING"  # No recent scans
            else:
                avg_time = sum(v["execution_time"] for v in recent_scans) / len(
                    recent_scans
                )
