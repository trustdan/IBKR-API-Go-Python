...
class OptionSelector:
    ...
    def select_vertical_spread(self, symbol: str, direction: str, current_price: float) -> Optional[OptionSpread]:
        ...
        try:
            ...
            if direction == "LONG":
                ...
                spreads = self.create_call_vertical_spreads(valid_expirations, current_price)
            else:
                # For bearish trades, find put verticals
                spreads = self.create_put_vertical_spreads(valid_expirations, current_price)
        ...
        for exp in expiration_dates:
            for strike in atm_strikes:
                ...
                call = Option(
                    ...
                    delta=max(0.01, min(0.99, 0.5 + (100 - strike) * 0.04)),  # Synthetic delta
                    gamma=0.02,
                    theta=-0.01,
                    vega=0.1,
                    rho=0.01
                ...
                put = Option(
                    ...
                    open_interest=1000,
                    implied_volatility=0.3,
                    delta=min(-0.01, max(-0.99, -0.5 - (strike - 100) * 0.04)),  # Synthetic delta
                    gamma=0.02,
                    theta=-0.01,
                    vega=0.1,
                    rho=0.01
    ...
    def filter_spreads(self, spreads: List[OptionSpread]) -> List[OptionSpread]:
        ...
        """
        return [
            spread for spread in spreads if 
            self.config.MIN_DELTA <= abs(spread.delta) <= self.config.MAX_DELTA and
            spread.cost <= self.config.MAX_SPREAD_COST and
            spread.reward_risk_ratio >= self.config.MIN_REWARD_RISK and
            spread.cost > 0  # Avoid zero or negative cost spreads (data errors)
        ]
                
    def create_call_vertical_spreads(self, option_chains: Dict[date, List[Option]], current_price: float) -> List[OptionSpread]:
        ...
        for expiry, chain in option_chains.items():
            ...
            calls = sorted([opt for opt in chain if opt.option_type == "call"], key=lambda x: x.strike)
    ...
    def create_put_vertical_spreads(self, option_chains: Dict[date, List[Option]], current_price: float) -> List[OptionSpread]:
        ...
        for expiry, chain in option_chains.items():
            ...
            puts = sorted([opt for opt in chain if opt.option_type == "put"], key=lambda x: x.strike)
