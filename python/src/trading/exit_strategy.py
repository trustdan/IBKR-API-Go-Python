...
class ExitStrategyManager:
    ...
    def calculate_exits(self, entry_price: float, direction: str, 
                       option_spread: OptionSpread, underlying_data: Optional[Dict] = None) -> Dict[str, float]:
        """
        ...
        # Profit target
        exits['profit_target'] = self._calculate_profit_target(entry_price, direction, option_spread)
    ...
    def _calculate_stop_loss(self, entry_price: float, direction: str) -> float:
        ...
        Returns:
            Stop loss price
        """
        stop_percentage = getattr(self.config, 'STOP_LOSS_PERCENTAGE', 0.5)  # Default 50%
    ...
    def _calculate_profit_target(self, entry_price: float, direction: str, option_spread: OptionSpread) -> float:
        """
        ...
        if direction == 'LONG':
            if option_spread.spread_type == 'BULL_CALL':
                ...
                max_profit = (option_spread.short_leg.strike - option_spread.long_leg.strike) - entry_price
            else:  # BEAR_PUT
                # Bear put spread: difference between strikes minus cost
                max_profit = (option_spread.long_leg.strike - option_spread.short_leg.strike) - entry_price
        else:  # SHORT
    ...
    def _calculate_r_multiple_exit(self, entry_price: float, direction: str, stop_loss: float) -> float:
        """
    ...
    def _calculate_atr_exit(self, entry_price: float, direction: str, underlying_data: Dict) -> float:
        """
    ...
    def _calculate_fibonacci_exit(self, entry_price: float, direction: str, option_spread: OptionSpread) -> float:
        """
        ...
        if direction == 'LONG':
            if option_spread.spread_type == 'BULL_CALL':
                price_range = (option_spread.short_leg.strike - option_spread.long_leg.strike) - entry_price
            else:  # BEAR_PUT
                price_range = (option_spread.long_leg.strike - option_spread.short_leg.strike) - entry_price
        else:  # SHORT
    ...
    def should_exit_position(self, position: Dict, current_price: float, 
                           days_to_expiry: int, underlying_data: Optional[Dict] = None) -> Tuple[bool, str]:
        """
        ...
        if getattr(self.config, 'USE_R_MULTIPLE_EXIT', False):
            stop_loss = position.get('stop_price', self._calculate_stop_loss(entry_price, direction))
            r_value = abs(entry_price - stop_loss)
            r_multiple = getattr(self.config, 'R_MULTIPLE_TARGET', 2.0)
    ...
    def _check_trailing_stop(self, position: Dict, current_price: float) -> Tuple[bool, str]:
        """
        ...
        # Trailing percentage
        trail_percentage = getattr(self.config, 'TRAILING_STOP_PERCENTAGE', 0.2)  # 20% by default
        ...
        if direction == 'LONG':
            ...
            if current_price <= trailing_stop:
                return True, f"Trailing stop triggered ({trail_percentage:.1%} from high of {highest_price:.2f})"
        else:  # SHORT
            ...
            if current_price >= trailing_stop:
                return True, f"Trailing stop triggered ({trail_percentage:.1%} from low of {lowest_price:.2f})"
    ...
    def update_exits_for_position(self, position: Dict, current_price: float,
                                underlying_data: Optional[Dict] = None) -> Dict:
        """
        ...
        if getattr(self.config, 'USE_TRAILING_STOP', False):
            if direction == 'LONG':
                ...
                if current_price > highest_price:
                    ...
                    trail_percentage = getattr(self.config, 'TRAILING_STOP_PERCENTAGE', 0.2)
