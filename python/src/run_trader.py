...
def main() -> int:
    ...
    scan_parser.add_argument('--symbols', required=True, help='File containing symbols to scan')
    scan_parser.add_argument('--strategies', nargs='+', 
    ...
    scan_parser.add_argument('--config', default='config.yaml', help='Path to config file')
    scan_parser.add_argument('--output', help='Output file for scan results (JSON)')
    
    # Trade command
    trade_parser = subparsers.add_parser('trade', help='Run the trader with scanning and execution')
    trade_parser.add_argument('--symbols', required=True, help='File containing symbols to trade')
    trade_parser.add_argument('--strategies', nargs='+', 
                         choices=['HIGH_BASE', 'LOW_BASE', 'BULL_PULLBACK', 'BEAR_RALLY'],
                         default=['HIGH_BASE', 'LOW_BASE', 'BULL_PULLBACK', 'BEAR_RALLY'],
                         help='Strategies to use for trading')
    trade_parser.add_argument('--config', default='config.yaml', help='Path to config file')
    trade_parser.add_argument('--interval', type=int, default=3600, 
                         help='Scanning interval in seconds (default: 3600)')
    trade_parser.add_argument('--cycles', type=int, default=1, 
    ...
    status_parser.add_argument('--config', default='config.yaml', help='Path to config file')
    ...
    try:
        if args.command == 'scan':
            ...
            log_info(f"Running scan with {len(symbols)} symbols and strategies: {args.strategies}")
            ...
            log_info(f"Scan completed, found {signal_count} signals across {len(results)} symbols")
            ...
            log_info(f"Starting trader with {len(symbols)} symbols and strategies: {args.strategies}")
            ...
            try:
                ...
                while max_cycles == 0 or cycle <= max_cycles:
                    ...
                    # Run scan cycle
                    results = trader.run_scan_cycle(symbols, args.strategies)
                    
                    log_info(f"Scan cycle {cycle} completed in {results['scan_time']:.2f}s, "
                           f"found {results['signals_found']} signals")
                    
                    # Print trader status
                    status = results['status']
                    log_info(f"Trader status: Mode={status['mode']}, "
                           f"Positions={status['risk']['active_positions']}/{status['risk']['max_positions']}, "
                           f"Daily trades={status['risk']['daily_trades']}/{status['risk']['max_daily_trades']}")
                    
                    # Wait for next cycle if not the last one
                    if max_cycles == 0 or cycle < max_cycles:
                        log_info(f"Waiting {args.interval} seconds until next scan cycle")
