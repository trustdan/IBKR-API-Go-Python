...
class DataManager:
    ...
    def __init__(self, ib_client: Optional[IB] = None) -> None:
        ...
        self.universe_cache_expiry = config.get("data.universe_cache_expiry", 1800)  # 30 minutes
        self.minute_data_cache_expiry = config.get("data.minute_data_cache_expiry", 60)  # 1 minute
        self.options_cache_expiry = config.get("data.options_cache_expiry", 300)  # 5 minutes
        
        # Universe filtering parameters
        self.min_market_cap = config.get("universe.min_market_cap", 10_000_000_000)  # $10B
    ...
    def bulk_get_data(
        ...
        if not self.scanner_client:
            log_warning("Scanner client not set, falling back to sequential data retrieval")
            return {symbol: self.get_data(symbol, timeframe, duration) for symbol in symbols}
        ...
        if duration == "1 Y":
            ...
            start_date = (now - timedelta(days=365)).strftime("%Y%m%d")  # Default to 1 year
        ...
        log_info(f"Bulk fetching data for {len(symbols_to_fetch)} symbols using scanner service")
    ...
    def filter_universe(
        ...
        for symbol in symbols:
            try:
                ...
                if ticker.last and ticker.last < self.min_price:
                    log_debug(f"{symbol} price {ticker.last} below minimum {self.min_price}")
                    continue
                    
                # Check volume criterion
                if ticker.volume and ticker.volume < self.min_volume:
                    log_debug(f"{symbol} volume {ticker.volume} below minimum {self.min_volume}")
                    continue
                ...
                if check_fundamentals:
                    ...
                    if "marketCap" in fundamentals:
                        market_cap_str = fundamentals.split("marketCap>")[1].split("</")[0]
                        try:
                            # Convert market cap to numeric value
                            market_cap = float(market_cap_str)
                            if market_cap < self.min_market_cap:
                                log_debug(f"{symbol} market cap {market_cap} below minimum {self.min_market_cap}")
                                continue
                        except ValueError:
                            log_warning(f"Could not parse market cap for {symbol}: {market_cap_str}")
        ...
        log_info(f"Filtered universe from {len(symbols)} to {len(filtered_symbols)} symbols")
